...
HGCalAnalysis::HGCalAnalysis(const edm::ParameterSet &iConfig)
    : readOfficialReco_(iConfig.getParameter<bool>("readOfficialReco")),
  ...
  if (!readOfficialReco_) {
    vtx_ = consumes<std::vector<TrackingVertex>>(edm::InputTag("mix", "MergedTrackTruth"));
    part_ = consumes<std::vector<TrackingParticle>>(edm::InputTag("mix", "MergedTrackTruth"));
  } else {
  ...
  t_->Branch("ecalDrivenGsfele_deltaEtaSuperClusterTrackAtVtx", &ecalDrivenGsfele_deltaEtaSuperClusterTrackAtVtx_);
  t_->Branch("ecalDrivenGsfele_deltaPhiSuperClusterTrackAtVtx", &ecalDrivenGsfele_deltaPhiSuperClusterTrackAtVtx_);
  t_->Branch("ecalDrivenGsfele_deltaEtaEleClusterTrackAtCalo", &ecalDrivenGsfele_deltaEtaEleClusterTrackAtCalo_);
  t_->Branch("ecalDrivenGsfele_deltaPhiEleClusterTrackAtCalo", &ecalDrivenGsfele_deltaPhiEleClusterTrackAtCalo_);
  t_->Branch("ecalDrivenGsfele_deltaEtaSeedClusterTrackAtCalo", &ecalDrivenGsfele_deltaEtaSeedClusterTrackAtCalo_);
  t_->Branch("ecalDrivenGsfele_deltaPhiSeedClusterTrackAtCalo", &ecalDrivenGsfele_deltaPhiSeedClusterTrackAtCalo_);
...
void HGCalAnalysis::analyze(const edm::Event &iEvent, const edm::EventSetup &iSetup) {
  ...
  if (!readOfficialReco_) {
    ...
    iEvent.getByToken(part_, partHandle);
    part = &(*partHandle);
  ...
  if (!readOfficialReco_) {
    ...
    for (unsigned int i = 0; i < npart; ++i) {
      ...
      // bunchCrossing == 0 intime, buncCrossing!=0 offtime, standard generation
      // has [-12,+3]
      if ((*part)[i].eventId().event() == 0 and (*part)[i].eventId().bunchCrossing() == 0) {
        // look for the generator particle, set to -1 for Geant produced
        // paticles
        int tp_genpart = -1;
        if (!(*part)[i].genParticles().empty()) tp_genpart = (*part)[i].genParticle_begin().key();

        // default values for decay position is outside detector_, i.e. ~stable
        int reachedEE = 1;
        double fbrem = -1;
        float dvx = 999.;
        float dvy = 999.;
        float dvz = 999.;
        if ((*part)[i].decayVertices().size() >=
            1) {  // they can be delta rays, in this case you have multiple
                  // decay verices
          dvx = (*part)[i].decayVertices()[0]->position().x();
          dvy = (*part)[i].decayVertices()[0]->position().y();
          dvz = (*part)[i].decayVertices()[0]->position().z();
          if ((*part)[i].decayVertices()[0]->inVolume())
            reachedEE = 0;  // if it decays inside the tracker volume
        }

        // look for origin of particle inside the beampipe so that we can assess
        // if a track can potentially give a reco::Track
        float r_origin = (*part)[i].parentVertex()->position().Pt();
        bool fromBeamPipe = true;
        if (r_origin > 2.0) fromBeamPipe = false;
        genpart_eta_.push_back((*part)[i].eta());
        genpart_phi_.push_back((*part)[i].phi());
        genpart_pt_.push_back((*part)[i].pt());
        genpart_energy_.push_back((*part)[i].energy());
        genpart_dvx_.push_back(dvx);
        genpart_dvy_.push_back(dvy);
        genpart_dvz_.push_back(dvz);
        genpart_fbrem_.push_back(fbrem);
        genpart_pid_.push_back((*part)[i].pdgId());
        genpart_gen_.push_back(tp_genpart);
        genpart_reachedEE_.push_back(reachedEE);
        genpart_fromBeamPipe_.push_back(fromBeamPipe);
      }
    }
  } else {
    ...
    for (unsigned int i = 0; i < npart; ++i) {
      ...
      int reachedEE = 0;  // compute the extrapolations for the particles
                          // reaching EE and for the gen particles
      double fbrem = -1;
      if (myTrack.noEndVertex() || myTrack.genpartIndex() >= 0) {
        ...
        BaseParticlePropagator myPropag(part, 160, layerPositions_[0], 3.8);
        myPropag.propagate();
        ...
        for (unsigned il = 0; il < nlayers; ++il) {
          myPropag.setPropagationConditions(140, layerPositions_[il], false);
          if (il > 0)  // set PID 22 for a straight-line extrapolation after the
                       // 1st layer
            myPropag.setID(22);
          myPropag.propagate();
  ...
  for (auto const &ele : electrons) {
    ...
    ecalDrivenGsfele_seedlayer_.push_back(recHitTools_.getLayerWithOffset(ele.superCluster()->seed()->seed()));
    ...
    ecalDrivenGsfele_deltaEtaSuperClusterTrackAtVtx_.push_back(ele.deltaEtaSuperClusterTrackAtVtx());
    ecalDrivenGsfele_deltaPhiSuperClusterTrackAtVtx_.push_back(ele.deltaPhiSuperClusterTrackAtVtx());
    ...
    ecalDrivenGsfele_deltaEtaSeedClusterTrackAtCalo_.push_back(ele.deltaEtaSeedClusterTrackAtCalo());
    ecalDrivenGsfele_deltaPhiSeedClusterTrackAtCalo_.push_back(ele.deltaPhiSeedClusterTrackAtCalo());
...
void HGCalAnalysis::computeWidth(const reco::HGCalMultiCluster &cluster, math::XYZPoint &bar,
                                 ...
                                 float &sige, float radius) {
  // bool recomputePCA_=false;
  sigu = 0.;
  sigv = 0.;
  sigp = 0.;
  sige = 0.;
    ...
    for (unsigned int j = 0; j < hfsize; j++) {
      ...
      if (fraction > 0) {
        ...
        if (local.Perp2() < radius2) {
          ...
          sige +=
              (rh_point.eta() - cluster.eta()) * (rh_point.eta() - cluster.eta()) * hit->energy();
          sigp += deltaPhi(rh_point.phi(), cluster.phi()) *
                  deltaPhi(rh_point.phi(), cluster.phi()) * hit->energy();
        ...
        // std::cout << " layer " << layer << " thickness " << thickness << "
        // mip " << mip << std::endl;
